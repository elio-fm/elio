use super::super::file_browser_pane::render_compact_list_row;
#[cfg(unix)]
use super::super::file_browser_pane::symlink_target_detail;
...
fn narrowing_horizontal_browser_layout_starts_shrinking_sidebar_early() {
    for (width, expected_sidebar_width) in [
        ...
        assert_eq!(
            layout.places.expect("sidebar should be visible").width,
...
fn narrow_legacy_layout_keeps_preview_without_starving_files() {
    for (width, height, expect_sidebar, expect_stacked) in [
        ...
        let entries = layout.file_browser.expect("entries should be visible");
        let preview = layout.preview.expect("preview should be visible");
        assert_eq!(layout.places.is_some(), expect_sidebar, "width {width}");
        if let Some(sidebar) = layout.places {
        ...
        if expect_stacked {
            ...
            assert_eq!(
                layout.places.map_or(0, |sidebar| sidebar.width) + entries.width + preview.width,
                width
            );
        }
    }
...
fn symlink_list_rows_expose_target_detail_inline() {
    ...
    assert_eq!(
        symlink_target_detail(entry).as_deref(),
