...
impl App {
    ...
    pub(super) fn prepare_static_image_for_overlay(
        &mut self,
        ...
        if is_png_path(&request.path) {
            let Some((width_px, height_px)) = image::image_dimensions(&request.path).ok() else {
                return StaticImageOverlayPreparation::Failed;
            };
            if width_px == 0 || height_px == 0 {
                return StaticImageOverlayPreparation::Failed;
            }
            let dimensions = RenderedImageDimensions {
                width_px,
                height_px,
            };
        ...
        let ffmpeg_available = self.ffmpeg_available();
        let magick_available = self.magick_available();
        if !self
            .scheduler
            .submit_image_prepare(jobs::ImagePrepareRequest {
...
pub(crate) fn prepare_static_image_asset(
    path: &Path,
    size: u64,
    modified: Option<SystemTime>,
    target_width_px: u32,
    ...
    let source_dimensions = if is_svg_path(path) {
        read_svg_dimensions(path)?
    } else {
    ...
    if is_svg_path(path) {
        ...
        if magick_available
            && render_svg_to_png(path, &cache_path, target_width_px, target_height_px)
        {
            return Some(PreparedStaticImageAsset {
                display_path: cache_path,
                dimensions: source_dimensions,
    ...
    if ffmpeg_available
        && should_extract_first_frame_with_ffmpeg(path)
...
fn render_first_frame_to_png_with_ffmpeg(
    ...
    Command::new("ffmpeg")
        ...
        .stderr(Stdio::null())
        .status()
        .is_ok_and(|status| status.success() && output_path.exists())
}
