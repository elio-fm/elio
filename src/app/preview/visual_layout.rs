...
impl App {
    ...
    fn preview_visual_failed_for_rows(&self, area: Rect, rows: u16) -> bool {
        ...
        self.preview
            .image
            .failed_images
            .contains(&static_images::StaticImageKey::from_request(&request))
    }
    ...
    pub(in crate::app) fn preview_visual_overlay_request_for_visual(
        ...
        area: Rect,
    ) -> static_images::StaticImageOverlayRequest {
        static_images::StaticImageOverlayRequest {
            path: visual.path.clone(),
            size: visual.size,
            modified: visual.modified,
            area,
            target_width_px: static_images::image_target_width_px(
                area,
                self.cached_terminal_window(),
            ),
            target_height_px: static_images::image_target_height_px(
                area,
                self.cached_terminal_window(),
            ),
            mode: static_images::StaticImageOverlayMode::Inline,
