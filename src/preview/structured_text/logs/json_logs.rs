use super::log_rendering::truncate_display;
use super::log_types::{LogSource, ParsedLogDocument, ParsedLogEntry};
    ...
    for (key, value) in map {
            ...
            )
        {
            level = json_scalar_to_string(&value)
                .and_then(|value| super::application_logs::canonical_level(&value));
            continue;
        }
        if message.is_none()
            && matches!(
