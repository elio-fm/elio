use super::super::{
    metadata_formatting::{format_unix_local, present_str, push_count_stat},
    metadata_preview::DocumentMetadata,
};
use std::{collections::BTreeMap, fs::File, io::Read, path::Path};

const DOC_SUMMARY_INFORMATION_STREAM: &str = "/\u{5}SummaryInformation";
const DOC_SUMMARY_INFORMATION_LIMIT_BYTES: usize = 64 * 1024;
...
pub(in crate::preview::documents) fn extract_legacy_word_metadata(
    path: &Path,
    ...
    let mut bytes = Vec::with_capacity(DOC_SUMMARY_INFORMATION_LIMIT_BYTES);
    stream
        .take(DOC_SUMMARY_INFORMATION_LIMIT_BYTES as u64)
