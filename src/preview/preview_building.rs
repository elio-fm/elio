    ...
    F: Fn() -> bool,
{
    ...
    if preview_spec.kind == file_classification::PreviewKind::Iso
        && let Some(preview) = archives::build_iso_preview(&entry.path)
    {
        return preview;
    }
    if preview_spec.kind == file_classification::PreviewKind::Torrent
        && let Some(preview) = torrent::build_torrent_preview(&entry.path)
    {
        return preview;
    }
    if facts.builtin_class == FileClass::Archive
    ...
    {
        if let Some(preview) = archives::build_archive_preview(
            &entry.path,
            type_detail,
    ...
    if let Some(document_format) = preview_spec.document_format
        && let Some(preview) = documents::build_document_preview(
            &entry.path,
            document_format,
    ...
    {
        return images::build_image_preview(entry, type_detail);
    }
    if facts.builtin_class == FileClass::Audio {
    ...
    if matches!(
        ...
        file_classification::PreviewKind::Sqlite
            | file_classification::PreviewKind::SqliteCandidate
    ) && let Some(preview) = tabular_data::build_sqlite_preview(&entry.path)
    {
        return apply_type_detail(preview, type_detail);
    }
    ...
    if preview_spec.kind == file_classification::PreviewKind::Csv {
        ...
        return tabular_data::build_csv_preview(
            &text_preview.text,
            is_tsv,
            type_detail,
    ...
    if preview_spec.kind == file_classification::PreviewKind::Source {
        if let Some(structured_format) = preview_spec.structured_format {
            let structured_attempt = structured_text::render_structured_preview(
                &text_preview.text,
                structured_format,
                text_preview.bytes_truncated,
            );
