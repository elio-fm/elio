use super::archive_cache::system_time_key;
use super::archive_reading::{CachedComicArchive, ComicArchiveBackend, ComicArchivePage};
use crate::preview::archives::archive_preview::ZIP_BUILT_IN_READER_MAX_BYTES;
    ...
    if !cache_path.exists() {
        ...
        let bytes = match comic.backend {
            ComicArchiveBackend::Zip => {
                ...
                if physical_size.is_some_and(|size| size > ZIP_BUILT_IN_READER_MAX_BYTES) {
                    return None;
                }
