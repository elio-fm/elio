...
pub(super) fn sample_iso_descriptors() -> Vec<u8> {
    let mut bytes = vec![
        0u8;
        (archive_preview::ISO_DESCRIPTOR_START_SECTOR + 3)
            * archive_preview::ISO_SECTOR_SIZE
    ];
    let start = archive_preview::ISO_DESCRIPTOR_START_SECTOR * archive_preview::ISO_SECTOR_SIZE;

    let boot = &mut bytes[start..start + archive_preview::ISO_SECTOR_SIZE];
    boot[0] = 0;
    boot[1..6].copy_from_slice(b"CD001");
    boot[6] = 1;
    write_iso_field(boot, 7, 39, archive_preview::ISO_BOOT_SYSTEM_ID);

    let primary = &mut bytes
        [start + archive_preview::ISO_SECTOR_SIZE..start + archive_preview::ISO_SECTOR_SIZE * 2];
    primary[0] = 1;
    ...
    put_iso_u16_le(primary, 128, archive_preview::ISO_SECTOR_SIZE as u16);
    ...
    let terminator = &mut bytes[start + archive_preview::ISO_SECTOR_SIZE * 2
        ..start + archive_preview::ISO_SECTOR_SIZE * 3];
    terminator[0] = 255;
