...
use super::outgoing::{DndOperation, URI_LIST_MIME};

#[derive(Clone, Debug, Eq, PartialEq)]
pub(in crate::terminal_runtime) enum KittyDndEvent {
    DropOffer {
        mime_index: u8,
...
#[derive(Default)]
pub(in crate::terminal_runtime) struct Osc72State {
    fields: DndFields,
    payload: Vec<u8>,
    active: bool,
}
...
#[cfg(test)]
pub(in crate::terminal_runtime) fn parse_osc72(sequence: &[u8]) -> Option<KittyDndEvent> {
    let mut state = Osc72State::default();
    parse_osc72_with_state(sequence, &mut state)
}

pub(in crate::terminal_runtime) fn parse_osc72_with_state(
    sequence: &[u8],
