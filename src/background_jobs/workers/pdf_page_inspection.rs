...
pub(in crate::background_jobs) struct PdfProbePool {
...
pub(in crate::background_jobs) struct PdfProbeJobKey {
...
impl PdfProbePool {
        ...
        for _ in 0..worker_count {
            ...
            workers.push(thread::spawn(move || {
                while let Some(request) = PdfProbeShared::pop(&shared) {
                    ...
                    let result = pdf::probe_pdf_page(&request.path, request.page)
    ...
    #[cfg(test)]
    pub(in crate::background_jobs) fn pending_keys(&self) -> Vec<PdfProbeJobKey> {
        let state = lock_unpoison(&self.shared.state);
        state
            .pending_current
            .iter()
